...
export class Events implements OnInit {
  ...
  onTypeChange(): void {
    ...
    if (this.resourceType === 'ROOM') {
      ...
      const request: ResourceLookupRpcRequest = { resourceType: this.resourceType, name: '', limit: 2000 };
  ...
  searchPerson(): void {
    ...
    this.error.set(null);
    const request: ResourceLookupRpcRequest = { resourceType: 'PERSON', name: this.personName.trim() };
    this.rpc.execute<ResourceInterface[]>('ResourceLookupRpcRequest', request).subscribe({
      next: (list) => {
        const person = (list ?? [])[0];
        if (!person) {
