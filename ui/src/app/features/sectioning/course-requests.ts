...
/**
 * Course Requests (student) — a focused port of the legacy GWT Course Requests
 * widget (StudentSectioningWidget, request-entry mode). This is the Scheduling
 * Assistant MINUS schedule building / enroll / weekly grid: it only lets a
 * student edit and save their primary + alternate course requests.
 *
 * Backend is the classic SectioningService RemoteService ("sectioning.gwt").
 * Flow: pick a session -> checkEligibility (gates actions, may require a PIN) ->
 * savedRequest(cx) loads the editable list -> edit primary + alternate requests
 * (course-finder autocomplete + free time + "or" alternatives + reorder +
 * per-request wait-list) -> Validate (checkCourses) -> Save (saveRequest).
 * A Degree Plans button (listDegreePlans + DegreePlanDialog) populates the list.
 *
 * Defensive by design: never throws on empty / eligibility-blocked / no-student
 * states; shows a message and keeps the null-student guard instead.
 */
@Component({
  ...
  styles: [
    `
      .cr-page { display: flex; flex-direction: column; gap: 0.75rem; }
      .search-bar { display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap; }
      .cr-select { min-width: 20rem; }
      .center { display: flex; justify-content: center; padding: 2rem; }
      .hint { color: var(--p-text-muted-color, #888); }
      .cr-inline-msg { display: block; margin: 0.25rem 0; }
      .inline-spin { width: 1.5rem; height: 1.5rem; }
      .cr-card-head { display: flex; align-items: center; gap: 0.5rem; font-weight: 600; padding: 0.75rem 1rem 0; }
      ...
      .cr-ac { flex: 1 1 auto; }
      .cr-ac ::ng-deep .p-autocomplete,
      .cr-ac ::ng-deep input { width: 100%; }
      .add-alt {
...
export class CourseRequests implements OnInit {
  ...
  /** Confirmation messages (isError) shown inline after a Validate. */
  ...
  save(): void {
    ...
    const req = this.buildRequest();
    // Validate first: block on errors, confirm on confirmations, then save.
    this.busy.set(true);
  ...
  private doSave(req: CourseRequestInterface): void {
    this.runBusy(
  ...
  startOver(): void {
    this.confirm.confirm({
      header: 'Clear Requests',
      message: 'Clear all course requests? This does not change your saved requests until you Save.',
      icon: 'pi pi-exclamation-triangle',
  ...
  private parseClock(s: string, ref?: number): number | null {
    ...
    if (ap === 'pm' || ap === 'p') { if (h < 12) h += 12; }
      ...
      // No meridiem: use school-hours heuristic (1..6 => pm), and keep end after start.
      if (h >= 1 && h <= 6) h += 12;
