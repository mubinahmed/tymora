...
import org.unitime.timetable.gwt.shared.UnassignedExamsInterface.ExamTypeInfo;
import org.unitime.timetable.gwt.shared.UnassignedExamsInterface.UnassignedExamsRequest;
import org.unitime.timetable.gwt.shared.UnassignedExamsInterface.UnassignedExamsResponse;
...
/**
 * Read-only backing bean for the legacy unassignedExams.action (Not-Assigned
 * Examinations) Struts page. Returns the applicable examination types for the
 * current academic session (to populate a selector) and the exams of the
 * selected type that have no committed period assignment
 * ({@code Exam.getAssignedPeriod() == null}), projected to string rows.
 * Permission-gated by {@link Right#Examinations} and
 * {@link Right#NotAssignedExaminations} (both Session qualified, matching the
 * legacy action). Served entirely from persisted data (no solver proxy).
 * Additive: introduces no changes to existing behavior.
 *
 * @author Angular migration
 */
@GwtRpcImplements(UnassignedExamsRequest.class)
public class UnassignedExamsBackend implements GwtRpcImplementation<UnassignedExamsRequest, UnassignedExamsResponse> {

	@Override
	public UnassignedExamsResponse execute(UnassignedExamsRequest request, SessionContext context) {
		...
		context.checkPermission(sessionId, "Session", Right.NotAssignedExaminations);

		UnassignedExamsResponse response = new UnassignedExamsResponse();
		response.setTitle("Not-Assigned Examinations");

		// Applicable exam types (mirror the legacy unassignedExams.action selector).
		...
		for (Exam exam : exams) {
			// Read-only: not-assigned exams are those with no committed period.
